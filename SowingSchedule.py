...
# SowingSchedule classes
sowing_columns = ['SowingSchedule_T1', 'SowingSchedule_T2', 'SowingSchedule_T3', 'SowingSchedule_T4',
                  'SowingSchedule_T5']
# Models
evaluation_models = {
...
overall_results = {}
for sowing_col in sowing_columns:
    print(f"\n### Analiz yapılıyor: {sowing_col} ###")

    filtered_data = dataframe[dataframe[sowing_col] == 1]
    target = filtered_data['GrainYield']
    features = filtered_data.drop(columns=['GrainYield'] + sowing_columns)
    # SMOTE
    ...
    plt.title(f"ROC Eğrileri: {sowing_col}")
    ...
    plt.savefig(f"roc_curve_{sowing_col}.png")  # Her sınıf için ROC eğrisi kaydediliyor
    print(f"ROC eğrisi kaydedildi: roc_curve_{sowing_col}.png")
    plt.show()

    overall_results[sowing_col] = pd.DataFrame(model_results).T
# Show all results
for sowing, results_df in overall_results.items():
    print(f"\n### Performans Sonuçları: {sowing} ###")
    print(results_df)
    results_df.to_csv(f"results_{sowing}.csv", index=False)
    print(f"Sonuçlar {sowing} için 'results_{sowing}.csv' dosyasına kaydedildi.")